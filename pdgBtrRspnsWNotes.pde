// Try getting different ranges by amplifying the signal and braketing the signal at4-4.02 or something so that events are triggered more carefully
// Midi control OPZ knobs
// Export .OBJ models to fabricated etc.  
...
void setup() {
  ...
  // Add a slider to control bandA
  cp5.addSlider("bandA")
    .setPosition(10, 100)
    .setSize(200, 20)
    .setRange(0, 16) // Set the range of the slider
    .setValue(bandA); // Set the default value
    
      // Add a slider to control bandA
  cp5.addSlider("s")
    .setPosition(10, 100)
    .setSize(200, 20)
    .setRange(0, numPoints/3) // Set the range of the slider
    .setValue(bandA); // Set the default value
  ...
  cp5.addSlider("bandB")
    .setPosition(10, 130)
    .setSize(200, 20)
    .setRange(0, numPoints/2) // Set the range of the slider
    .setValue(bandB); // Set the default value
  ...
  cp5.addSlider("bandC")
    ...
    .setSize(200, 20)
    .setRange(0, numPoints/3) // Set the range of the slider
    .setValue(bandC); // Set the default value
...
void draw() {
  ...
  float speedFactor = map(spectrum[bandA], 0, .001, 1, 1);
  //float forceFactor = map(spectrum[900], 9, 9.01, 1, .009);//maybe around E5
  //float forceFactor = map(spectrum[15], 0, .05, 1, .009);//maybe around E5
